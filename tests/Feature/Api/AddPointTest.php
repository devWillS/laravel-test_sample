...
class AddPointTest extends TestCase
{
    ...
    /**
     * @test
     * @dataProvider dataProvider_put_add_point事前条件エラー
     */
    public function put_add_point事前条件エラー(int $addPoint)
    {
        $response = $this->putJson('/api/customers/add_point', [
            'customer_id' => self::CUSTOMER_ID,
            'add_point' => $addPoint,
    ...
    public function dataProvider_put_add_point事前条件エラー(): array
    {
        return [
            [0],
            [-1],
    ...
    public function put_add_point_customer_id事前条件エラー()
    {
        ...
        $response->assertStatus(400);
        $expected = [
            'message' => 'customer_points.customer_id:999 does not exists'
        ];
        $response->assertExactJson($expected);
    }
}
